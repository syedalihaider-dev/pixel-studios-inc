...
const linked = (href, text) => <Link href={href}>{text}</Link>;
...
const services = [
  ...
  contentParagraph: <>{text} {linked(href, link)} for style examples and turnaround timelines.</>,
}));
...
const solutions = [
  ...
  ["Visualize Complex Ideas", ["3D Product Visualization|Some products are difficult to understand from a photo or a written spec, especially before they're manufactured. We build 3D product visualizations for manufacturing industries that show a device from every angle, in motion, before a single physical unit exists, useful for pre-launch marketing, investor materials, and manufacturing partner conversations alike.", "Technical Animation|When a process, mechanism, or system is too complex to explain in text, technical animation breaks it down step by step, in a sequence an audience can actually follow. We build this kind of animation for hardware, engineering, and manufacturing-adjacent Austin businesses that need to explain how something works, not just what it looks like.", "Motion Graphics & Data Visualization|Numbers and data are hard to make compelling on a static slide. For Fintech and Financial service industries we build motion graphics that animate charts, metrics, and comparisons in a way that guides an audience through the story the data is telling, useful for investor updates, sales presentations, and internal reporting alike."]],
  ["Educate & Train Audiences", ["Employee Training Videos|Fast-growing Austin teams onboard new hires constantly, and repeating the same training conversation in person doesn't scale. We build employee training videos that cover process, policy, and product knowledge once, so new team members can ramp up on their own schedule without pulling a manager into another repeat session.", "Customer Education|Customers who understand your product fully tend to churn less and generate fewer support tickets. We build customer education content that walks users through setup, features, and troubleshooting in a format that's easier to follow and easier to reuse than a written help article.", "Healthcare & Educational Animation|Healthcare and education-adjacent Austin organizations often need to explain sensitive or technical information clearly and accurately. We build animation for patient education and instructional content that simplifies complex material without oversimplifying the facts, a balance that matters when accuracy is non-negotiable."]],
...
].map(([title, items]) => ({ title, subItems: items.map((item, index) => { const [subTitle, description] = item.split("|"); let finalDescription = description; if (subTitle === "SaaS & Software Explainers") { finalDescription = <>SaaS products are hard to explain because the value often lives inside a workflow, not a screenshot. We build {linked('/saas-explainer-videos', 'software explainers animations')} that show your platform in motion, walking through a real use case instead of listing features in a bulleted list. These videos work equally well embedded on a pricing page or sent directly to a prospect mid-sales-cycle.</>; } else if (subTitle === "3D Product Visualization") { finalDescription = <>Some products are difficult to understand from a photo or a written spec, especially before they're manufactured. We build {linked('/manufacturing-animation', '3D product visualizations for manufacturing industries')} that show a device from every angle, in motion, before a single physical unit exists, useful for pre-launch marketing, investor materials, and manufacturing partner conversations alike.</>; } return { image: images[index], title: subTitle, description: finalDescription, buttonText: "Request a Proposal", buttonLink: "/contact-us" }; }) }));
